...
public class UserMapper {
    ...
    public User mapUser(UserDTO user){
        return new User(user.getId(), user.getUsername(), user.getEmail(), user.getPassword(), user.getHighscores());
    }

    public UserDTO mapUserAndConcealData(User user){
        return new UserDTO(user.getId(), user.getUsername(), user.getHighscores());
    }
